...
from .utils import download_s3_to_file, write_parquet_async, upload_file_to_s3
from .manifest import upload_manifest_entry
from .config import OUTPUT_S3_BUCKET, OUTPUT_S3_PREFIX, DYNAMO_TABLE_NAME, PARQUET_MAX_CHUNK_SIZE_MB
from .aws_clients import dynamodb_client
import aioboto3

logger = logging.getLogger("pdf_processor.processor")

def _now_iso():
    return datetime.utcnow().isoformat() + "Z"

async def _get_dynamo_item(ddb, s3_key: str):
    resp = await ddb.get_item(TableName=DYNAMO_TABLE_NAME,
                              Key={"s3_key": {"S": s3_key}},
                              ConsistentRead=True)
...
async def process_pdf_per_page(s3_bucket_uri: str, s3_key: str, writer: ParquetBatchWriter, ddb_client=None):
    ...
    if ddb_client is None:
        ...
        ddb_client = await session.client("dynamodb").__aenter__()  # we will not close to keep calling functions simple
